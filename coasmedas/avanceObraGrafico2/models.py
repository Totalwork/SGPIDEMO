...
class CatalogoUnidadConstructiva (models.Model):
	...
	mcontrato = models.ForeignKey(Contrato,related_name="catalogo_contrato",null=True, blank=True, on_delete=models.PROTECT)
...
class EReformado(models.Model):
	fecha_registro = models.DateField(null=True,blank=True)
	usuario_registro = models.ForeignKey(Usuario,related_name='reporte_reforamado_usuario',on_delete=models.PROTECT)		
...
class EReformadoDetalle(models.Model):
	reformado = models.ForeignKey(EReformado,related_name='reporte_reforamado_fk',on_delete=models.PROTECT)	
	codigo_uucc = models.TextField(null=True,blank=True)
	descripcion_uucc = models.TextField(null=True,blank=True)
	...
	apoyo = models.ForeignKey(HNodo, related_name='reforma_nodo',on_delete=models.PROTECT)		
...
class JReporteTrabajo(models.Model):
	...
	#soporteAprobacion = models.FileField(upload_to=RandomFileName('avanceObraGrafico/soporte_aprobacion','aog'),blank=True, null=True)
...
class MLiquidacionUUCC(models.Model):
	estados=(
		(1, 'Abierta'),
		(2, 'Cerrada'),
		(3, 'Anulada')
	...
	presupuesto = models.ForeignKey(EPresupuesto,related_name='liquidacionuucc_presupuesto',on_delete=models.PROTECT, null=True, blank=True)
	detallereportetrabajo= models.ManyToManyField(KDetalleReporteTrabajo,related_name='fk_liquidacion_detallereportetrabajo')
	motivo_anulacion = models.CharField(max_length=150, null=True, blank=True)
	
	class Meta:
		db_table='avanceObraGrafico2_liquidacionuucc'
		ordering=['id']
		permissions=(
			('can_see_mliquidacionuucc','can see mliquidacionuucc'),
		)		
	...
	def __str__(self):
		return str(self.presupuesto.nombre+'_'+str(self.id)+'_'+str(self.estado))
...
class LSoporteCambio(models.Model):
	...
	#soporte = models.FileField(upload_to=RandomFileName('avanceObraGrafico/soporte_cambio','aog'),blank=True, null=True)
...
class FotoNodo (models.Model):
	nodo = models.ForeignKey(HNodo,
		related_name= 'fotoNodo_nodo',on_delete=models.PROTECT)
	fecha = models.DateField()
	# ruta  = models.FileField(
	# 	upload_to=RandomFileName(
	# 		'administrador_fotos/fotos_proyecto',''),
	# 		 blank=True, null=True, verbose_name='Ruta') 
	ruta  = models.FileField(upload_to='administrador_fotos/fotos_proyecto',
			blank=True, null=True, verbose_name='Ruta') 
	comentario = models.TextField(null=True,blank=True)

	def proyecto(self):
		return self.nodo.presupuesto.cronograma.proyecto.nombre
