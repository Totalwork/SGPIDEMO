...
# from .models import Contrato, VigenciaContrato
from factura.enumeration import estadoFactura, notificacion

from parametrizacion.models import Funcionario#, Notificacion
...
class FunctionTask:
    ...
    def facturasSinContabilizar():
        try:
            ...
            queryset = Factura.objects.filter(qset_por_contabilizar)

            noti = Funcionario.objects.filter(notificaciones=notificacion.cod, activo=1)
            ...
            for n in noti:
                ...
                facturas = []
                for	f in queryset:
                    # poliza = Factura.objects.get(pk=f.id)
                    verificar = esSoloLectura(f.contrato, n.empresa.id)
                    if verificar == False:
                        facturas.append(f)
                if facturas.count() > 0:
                    ...
                    contenido+='Estimado usuario(a), <br/><br/>nos permitimos comunicarle que las siguientes facturas estan <strong>sin contabilizar</strong>:<br/><br/>'
                    ...
                    mail = Mensaje(
                        ...
                        destinatario=n.persona.correo,
                        asunto='Facturas sin contabilizar',
                        contenido=contenido,
                        appLabel='factura',
                        tieneAdjunto=True,
