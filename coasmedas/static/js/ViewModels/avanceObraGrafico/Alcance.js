function IndexViewModel() {
   ...
   // self.url=path_principal+'api/Banco';   
     ...
     self.id_proyectos=ko.observable(0);

     self.listado_esquema=ko.observableArray([]);
     self.id_esquema=ko.observable(0);
    ...
    self.abrir_apoyo=function(obj){
        
       location.href=path_principal+"/avanceObraGrafico/apoyo/"+obj.id+"/";
    }



    self.vista_cantidad_apoyo=function(obj){

        path = path_principal+'/api/avanceObraGraficoNodo/?sin_paginacion';
        parameter = { presupuesto_id:obj.id};
        RequestGet(function (datos, estado, mensage) {
                
            if (estado=='ok' && datos.length>0) {
                        
                    location.href=path_principal+"/avanceObraGrafico/cantidad_apoyo/"+obj.id+"/";
            }else{
                 $.confirm({
                    title:'Informativo',
                    content: '<h4><i class="text-info fa fa-info-circle fa-2x"></i>Debe crear primero los apoyos para crear las cantidades.<h4>',
                    cancelButton: 'Cerrar',
                    confirmButton: false
                });
    ...
    self.consultar = function (pagina) {
        if (pagina > 0) {            
            ...
            sessionStorage.setItem("proyecto_id",self.id_proyectos() || 0);            
            sessionStorage.setItem("esquema_id",self.id_esquema() || 0);
    ...
    self.cargar =function(pagina){           
            ...
            let proyecto_id=sessionStorage.getItem("proyecto_id");
            let esquema_id=sessionStorage.getItem("esquema_id");

            path = path_principal+'/api/avanceObraGraficoPresupuesto/?format=json&page='+pagina;
            parameter = { alcance:0,empresa:$('#id_empresa').val(),dato: filtro_avance, pagina: pagina,
            proyecto_id:proyecto_id,departamento_id:departamento_id_avance,municipio_id:municipio_id_avance,esquema_id:esquema_id};
            RequestGet(function (datos, estado, mensage) {
    ...
    self.id_macrocontrato.subscribe(function(value){
        ...
        if(value!=0 && value!='' && value!=null){
            ...
            parameter = {'contrato_id':value};
            RequestGet(function (datos, estado, mensage) {
                
                if (estado=='ok') {
                    self.listado_contratista(datos.data.contratistas);
                    self.listado_departamento(self.llenar_departamento(datos.data.departamentos));
                    self.listado_municipio(datos.data.municipios);
                    self.listado_proyectos(datos.data.proyectos);
                    self.consultar_esquema(value);
            ...
            self.listado_esquema([]);
        }

    });


    self.consultar_esquema=function(mcontrato){

        path = path_principal+'/api/avanceObraGraficoEsquemaCapitulos/?sin_paginacion';
            parameter = {'macrocontrato_id':mcontrato};
            RequestGet(function (datos, estado, mensage) {
                ...
                if (estado=='ok') {
                    ...
                    self.id_esquema(sessionStorage.getItem("esquema_id"));       
            });
    }
   ...
   self.llenar_departamento=function(data){

        var lista=[];
        ko.utils.arrayForEach(data, function(d) {
                  lista.push({
                        id:d.municipio__departamento__id,
