function EnlaceViewModel(){
  ...
  self.nodoVO={
   ...
   self.cambioCronogramaVO={
    ...
    self.consultar_tendidos=function(id){
        ...
        path = path_principal+'/api/avanceObraGraficoEnlace/?sin_paginacion';
        parameter = {cronograma_id:$('#id_cronograma').val(),nodo_origen:id};
        RequestGet(function (datos, estado, mensage) {
    ...
    self.eliminar_tendidos=function(){
         ...
         if(count==0){
             ...
             var path =path_principal+'/avanceObraGrafico/eliminar_id_nodo_destino/';
    ...
    self.consultar_detalle=function(id){
          ...
          parameter = {cronograma_id:$('#id_cronograma').val(),listado_apoyo:1,id_apoyo:id};
          RequestGet(function (datos, estado, mensage) {
    ...
    self.crear_tendido=function(){
        if(self.apoyoVO.detallepresupuesto_id()==0 || self.apoyoVO.nodoDestino_id()==0){
            ...
            path = path_principal+'/api/avanceObraGraficoEnlace/?sin_paginacion';
            parameter = {cronograma_id:$('#id_cronograma').val(),nodo_origen:self.apoyoVO.nodoOrigen_id(),nodo_destino:self.apoyoVO.nodoDestino_id()};
            RequestGet(function (datos, estado, mensage) {
                   ...
                   if(datos.length==0){
                        ...
                        var parametros={                     
                             ...
                             url:path_principal+'/api/avanceObraGraficoEnlace/',//url api
    ...
    self.cargar = function () {
            ...
            path = path_principal+'/avanceObraGrafico/consultar_avance_obra/';
            parameter = {ejecucion:self.filtro_ejecutado(),cronograma_id:$('#id_cronograma').val(),id_estados:self.id_estado_busqueda(),dato:filtro_avance,programando:programado};
            RequestGet(function (datos, estado, mensage) {
  ...
  ko.utils.arrayForEach(self.listado(), function(d) {
        ...
        marker.addListener('click', function(e) {
            ...
            '<div id="bodyContent">'+
            '<p><button onclick="llamarDetalle('+d.id+')">Ingresar Datos</button></p>'+
            '<p><button onclick="detalleDatos('+d.id+')">Detalles de los Cambios</button></p>'+
            '<p><button onclick="llamarTendido('+d.id+')">Crear tendido</button></p>'+
            '<p><button onclick="detalleTendido('+d.id+')">Detalle del tendido</button></p>';

            if(self.cambio_eliminar()==true){
               contentStringOrg=contentStringOrg+ '<p><button onclick="eliminarpunto('+d.id+')">Eliminar Apoyo...</button></p>';
            }

            if(self.cambio_modificar()==true){
               contentStringOrg=contentStringOrg+ '<p><button onclick="modificarpunto('+d.id+')">Modificar Apoyo...</button></p>';
            }
  ...
  self.guardar_nodo=function(latLng){
      ...
      if (EnlaceViewModel.errores_apoyo().length == 0) {//se activa las validaciones
           ...
           // self.contratistaVO.logo($('#archivo')[0].files[0]);            
                self.nodoVO.latitud(latLng.lat());
                self.nodoVO.longitud(latLng.lng());
