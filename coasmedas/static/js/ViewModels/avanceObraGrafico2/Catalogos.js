function IndexViewModel() {
    ...
    self.cargar =function(pagina){           
        ...
        path = path_principal+'/api/catalogounidadconstructiva/?format=json&page='+pagina;
    ...
    self.carga_excel=function(){
        ...
        if(self.archivo_carga()==''){
          ...
          var parametros={                     
            ...
            url:path_principal+'/avanceObraGrafico2/carga_masiva_catalogo/',
            parametros:data                        
          };
         ...
         if (IndexViewModel.errores_catalogos().length == 0) {//se activa las validaciones           
            if(self.catalogo.id()==0){
                var parametros={                     
                    ...
                    url:path_principal+'/api/catalogounidadconstructiva/',//url api
               ...
               RequestFormData(parametros);                                
                ...
                var parametros={
					...
					url:path_principal+'/api/catalogounidadconstructiva/'+self.catalogo.id()+'/',
					parametros:self.catalogo                        
				};
				RequestFormData(parametros);			
            }
    ...
    self.exportar_excel=function(){
		location.href=path_principal+"/avanceObraGrafico2/excel_catalogo/?dato="+self.filtro()
    } 
    
    self.consultar_por_id=function(obj){
        self.titulo('Actualizar Catalogo');
        path = path_principal+'/api/catalogounidadconstructiva/'+obj.id+'/?format=json';
        parameter = {};
        RequestGet(function (datos, estado, mensage) {
    ...
    self.consultar_uucc = function(obj){        
        location.href=path_principal+"/avanceObraGrafico2/uucc/"+obj.id+"/";
    }

    self.consultar_materiales = function(obj){        
        location.href=path_principal+"/avanceObraGrafico2/materiales/"+obj.id+"/";
    }

    self.consultar_manoObra = function(obj){        
        location.href=path_principal+"/avanceObraGrafico2/mano_obra/"+obj.id+"/";
    }    
