function IndexViewModel() {
   ...
   self.listado_rechazo=ko.observableArray([]);
    ...
    self.eliminar=function(){
        var lista_id=[];
         ...
         if(count==0){
              ...
              $.confirm({
                title:'Informativo',
                content: '<h4><i class="text-info fa fa-info-circle fa-2x"></i>Seleccione un reformado para la eliminacion.<h4>',
                cancelButton: 'Cerrar',
                confirmButton: false
            });

         }else{
             var path =path_principal+'/avanceObraGrafico2/eliminar_id_reformado/';
             var parameter = { lista: lista_id};
             RequestAnularOEliminar("Esta seguro que desea eliminar los reformados seleccionados?", path, parameter, function () {
                 self.consultar(1);
                 self.checkall(false);
             })
    ...
    self.cargar =function(pagina){           
            ...
            parameter = {dato: filtro_avance, pagina: pagina,presupuesto_id:$("#presupuesto_id").val(),empresa_id:$('#id_empresa').val(), reformado: 1};
            RequestGet(function (datos, estado, mensage) {
    ...
    self.reformado=function(obj){
        ...
        location.href=path_principal+"/avanceObraGrafico2/reformadoDetalle/"+obj.id+"/";
    } 
