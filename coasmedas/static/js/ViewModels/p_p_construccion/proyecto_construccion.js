...
function ProyectoConstruccionViewModel() {
    ...
    //funcion para filtrar en el administrador de fotos
   ...
   //consultar los macrocontrato
    self.consultar_macrocontrato=function(){
         ...
         RequestGet(function (datos, estado, mensaje) {
            ...
            self.lista_contrato(datos.macrocontrato);
             ...
             self.macrocontrato_select(sessionStorage.getItem("mcontrato_filtro_administrador_fotos"));
         },false,false);
        ...
        if (value>0) {
             ...
             RequestGet(function (datos, estado, mensaje) {
                ...
                self.listado_contratista(datos.contratista);
                     ...
                     self.contratista(sessionStorage.getItem("contratista_filtro_administrador_fotos"));       
                 });
        }
        ...
        if (contrato>0) {
             ...
             RequestGet(function (results,count) {
                ...
                self.listado_departamento(results.departamento);    
                         ...
                         self.departamento(sessionStorage.getItem("departamento_administrador_fotos"));       
                     });
        }
        ...
        if (departamento>0) {
             ...
             RequestGet(function (results,count) {
                ...
                self.listado_municipio(results.municipio);       
                     ...
                     self.municipio(sessionStorage.getItem("municipio_administrador_fotos"));       
                 });
        }
    ...
    self.consultar = function (pagina) {
        ...
        if (pagina > 0) {            
            ...
            sessionStorage.setItem("dato_proyecto_construccion", $('#txtBuscar').val() || '');
            sessionStorage.setItem("municipio_proyecto_construccion", $('#municipio_filtro').val() || '');
            sessionStorage.setItem("departamento_proyecto_construccion", $('#departamento_filtro').val() || '');
            sessionStorage.setItem("contratista_filtro_proyecto_construccion", $('#contratista_filtro').val() || '');            
            sessionStorage.setItem("mcontrato_filtro_proyecto_construccion", $('#mcontrato_filtro').val() || '');
    ...
    self.cargar = function(pagina){
        ...
        let filtro = sessionStorage.getItem("dato_proyecto_construccion");
        let municipio = sessionStorage.getItem("municipio_proyecto_construccion");
        let departamento = sessionStorage.getItem("departamento_proyecto_construccion");
        let contratista_filtro = sessionStorage.getItem("contratista_filtro_proyecto_construccion") 
        let mcontrato_filtro = sessionStorage.getItem("mcontrato_filtro_proyecto_construccion");

        var empresa=$("#empresa").val();

        path = path_principal+'/api/Proyecto_empresas_lite?format=json';
            parameter = { dato: self.filtro(), page: pagina, 
                mcontrato:mcontrato_filtro, 
                contratista:contratista_filtro, departamento:departamento, 
                municipio:municipio,empresa:empresa};
            RequestGet(function (datos, estado, mensage) {

                if (estado == 'ok' && datos.data!=null && datos.data.length > 0) {
                    self.mensaje('');
                    $('#filtrar_proyecto').modal('hide'); 
...
var proyecto_construccion = new ProyectoConstruccionViewModel();
ko.applyBindings(proyecto_construccion);
