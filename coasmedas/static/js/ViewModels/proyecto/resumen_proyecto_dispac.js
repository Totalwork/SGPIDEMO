...
function ResumenViewModel() {
   ...
   //consultar la api proceso relacion
    self.procesoRelacion=function(proceso_id,apuntador,proyecto_id){
        ...
        parameter={ proceso: proceso_id, verDetalle:1, apuntador:apuntador, idApuntador:proyecto_id };
        RequestGetAutenticacion(function (datos, estado, mensaje) {
    ...
    self.list_seguimiento=function(){
         ...
         parameter={ procesoRelacion:self.procesoRelacionId() };
         RequestGetAutenticacion(function (datos, estado, mensaje) {
            
            if (datos.data.listado[0]!=null && datos.data.listado.length > 0) {

                self.mensaje_listado_seguimiento('');
                self.listado_seguimiento(datos.data.listado);
                self.porcentaje_seguimiento(datos.data.etiquetaPorcentaje);
                self.porcentaje_se(datos.data.porcentaje);
    ...
    self.consultar_giros_contrato = function (proyecto) {
        ...
        RequestGetAutenticacion(function (datos, estado, mensage) {
            ...
            if (datos!=null && datos.length > 0) {
                ...
                self.mensaje_giros_contrato(mensageNoFound(mensage));
            }
    ...
    self.consultar_categoria = function (id_proyecto) {        
     ...
     self.consultar_subcategoria = function () {
        ...
        if(lista_id !=''){
            ...
            parameter = {lista:lista_id,proyecto:self.proyecto()};
            RequestGetAutenticacion(function (datos, estado, mensage) {
