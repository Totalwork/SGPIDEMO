...
function FacturaPagadaRecursosPropiosViewModel() {
    ...
    self.paginacion.pagina_actual.subscribe(function (pagina) {
        self.consultar_pago_factura_recursos_propios(pagina);
    });
    ...
    self.consultar_factura_pagada = function (obj) {
        ...
        self.titulo('Listado de facturas pagadas');
        var bloqueo_factura=0;
    ...
    self.guardar=function(){
        ...
        var parametros={     
            ...
            completado:function(){
             self.consultar_pago_factura_recursos_propios(1);
            }                         
        };
    ...
    self.consultar_pago_factura_recursos_propios = function (pagina) {
        ...
        if (pagina > 0) { 
            ...
            parameter = { dato: self.filtro(), page: pagina, contrato:contrato, contratista:contratista, 
                            cuenta:cuenta, desde:self.desde(), hasta:self.hasta(), recursos:1};
            RequestGet(function (datos, estado, mensage) {
    ...
    self.consultar_macrocontrato=function(){
         ...
         path = path_principal+'/api/gestion_op?format=json&sin_paginacion=';
         parameter={ recursos:1 };
         RequestGet(function (datos, estado, mensaje) {
            //console.log(datos.data.contrato)   
   ...
   self.exportar_excel_gestion=function(){
        var recursos=1;
        location.href=path_principal+"/seguimiento_factura/exportar_gestion_op_recursos_propios/?recursos="+recursos;
    }
    ...
    self.anular_compensacion = function(obj){

        var path = path_principal+'/seguimiento_factura/anular-pago-compensacion/';
        var parameter = { gestion_op_id : obj.id };
        RequestAnularOEliminar(" Esta seguro que desea anular el pago seleccionado?", path, parameter, function () {
            alert("debug")
            self.consultar();
        })
...
var consultar_pago_factura_recursos_propios = new FacturaPagadaRecursosPropiosViewModel();
ko.applyBindings(consultar_pago_factura_recursos_propios);
